...
class NeuralISMCTSPUCTAgent:
    """
    MLP-based neural-guided ISMCTS + PUCT.

    Design choices:
    - Network takes a flat observation encoding.
    - Root priors are computed once per real move.
    - Deeper priors are recomputed per simulation-local observation.
    - Per-move prior cache reduces repeated neural forward passes when the same
      encoded observation appears multiple times.
    - Rewards are backed up from the root player's perspective.
    """
    ...
    def search_policy(
        ...
        # Per-move prior cache: exact with respect to encoder output
        prior_cache: Dict[Any, Dict[Action, float]] = {}

        # Root priors only depend on the real observation, so compute once
    ...
    def _make_prior_cache_key(self, flat_features: List[float]) -> Any:
        """
        Cache key for priors. Safe because the MLP policy depends only on the
        encoded flat observation vector.
        """
        return tuple(flat_features)
    ...
    def _compute_priors_from_obs_inplace(
        ...
        flat_features = self.encoder.encode(obs)
        cache_key = self._make_prior_cache_key(flat_features)
