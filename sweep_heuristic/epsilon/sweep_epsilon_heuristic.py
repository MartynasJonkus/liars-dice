...
def run_single_game(args):
    ...
    agent_specs = [
        (agent_paths[0], {"rollout_eps": value}),   # ISMCTS tuned
...
def run_c_sweep_parallel(
    ...
    for v in values:
        print(f"\n=== Running sweep for epsilon = {v} ({num_games} games) ===")
    ...
    plt.title("ISMCTS-Heuristic Average placement vs Rollout epsilon constant")
    plt.xlabel("Rollout epsilon constant")
