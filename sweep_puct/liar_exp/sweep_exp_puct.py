...
def run_single_game(args):
    ...
    agent_specs = [
        (agent_paths[0], {"liar_exp": value}),  # ISMCTS tuned
    ...
    game = LiarsDiceGame(
        num_players=3, dice_per_player=dice_per_player, seed=rng.randint(0, 10**9)
    )
    ...
    return placements.index("ISMCTS-PUCT") + 1
...
def run_c_sweep_parallel(
    ...
    for v in values:
        print(f"\n=== Running sweep for liar_exp = {v} ({num_games} games) ===")
        ...
        avg = (
            1 * placement_counts[1] + 2 * placement_counts[2] + 3 * placement_counts[3]
        ) / total
    ...
    plt.title("ISMCTS-PUCT Average placement vs Prior constant exp")
    plt.xlabel("Prior constant exp")
    plt.ylabel("Average placement (lower is better)")
    plt.ylim(1, 3)
    plt.yticks([1, 2, 3])
    for v, avg in zip(df["value"], df["avg"]):
        plt.text(v, avg, f"{avg:.2f}", ha="center", va="bottom", fontsize=9)
    plt.grid(True)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Parallel sweep for ISMCTS-PUCT")
    ...
    VALUES = [0.5, 1.0, 1.25, 1.5, 2.0, 3.0]
