...
def play_one_game_sweep(
    agent_paths: List[str], sims_count: int, dice_per_player: int, seed: int
):
    rng = random.Random(seed)

    agents_with_params = [
        load_agent_with_kwargs(
            agent_paths[0], seed=rng.randint(0, 10**9), sims_per_move=sims_count
        ),
    ...
    while True:
        ...
        if action not in legal:
            raise RuntimeError(
                f"Illegal action {action} by agent {names[pid]}. Legal: {legal}"
            )
...
def run_c_sweep(
    ...
    plt.title(
        "ISMCTS-PUCT Placement distribution vs number of child nodes expanded per move"
    )
...
if __name__ == "__main__":
    ...
    S_VALUES = [100, 200, 500, 1000, 2000]
